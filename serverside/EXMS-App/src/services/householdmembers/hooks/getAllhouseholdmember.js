    ...
    if (context.params.users) {
      ...
      const filteredHouseholdMembers = householdmembers.filter((eachHouseholdMember) => {
        for (let eachfilteredhousehold of filteredHouseholds) {
          if (
            eachfilteredhousehold.household.equals(eachHouseholdMember.household) &&
            !eachfilteredhousehold.user.equals(eachHouseholdMember.user)
          )
            return true
        }
      })
      ...
      const householdmembersData = filteredHouseholdMembers.map((eachHouseholdmember) => {
        let member = {
          _id: eachHouseholdmember._id,
          householdId: eachHouseholdmember.household,
          memberUserId: eachHouseholdmember.user
        }

        const household = households.data.find((eachHousehold) =>
          eachHousehold._id.equals(eachHouseholdmember.household)
        )
